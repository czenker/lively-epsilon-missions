...
My.Translator:register("de", {
    ...
    upgrade_speed1_description = function(speedUPerMin)
        return f("Der Impulseantrieb MkI ist die Einsteigervariante für kleinere und mittlere Frachter. Geschwindigkeiten über %0.1fu/min sind nur für erfahrene Techniker machbar, aber aufgrund seiner Zuverlässigkeit wird der Antrieb überall geschätzt.", speedUPerMin)
    end,

    upgrade_speed2_name = "Impulsantrieb MkII",
    upgrade_speed2_description = function(speedUPerMin)
        return f("Der Impulseantrieb MkII ist eine verbesserte Version des serienmäßig in leichten Schiffen verbauten Antriebs. Im Normalbetrieb sind mit ihm Geschwindigkeiten bis zu %0.1fu/min möglich - ein guter Techniker kann aber auch mehr herauskitzeln.", speedUPerMin)
    end,
    ...
    upgrade_speed4_description = function(speedUPerMin)
        return f("Kleinere Verbesserungen an der Materieeinspritzung und die Optimierung des Wirkungsgrads machen den Impulsantrieb MkIV zu einer der besten Optionen für kleine und mittlere Frachter. Seine Höchstgeschwindigkeite liegt bei %0.1fu/min ohne Turbo.", speedUPerMin)
    end,
    ...
    upgrade_warpDrive_name = "Warp Antrieb X8",
    upgrade_warpDrive_description = "Für längere Reisen sind größere Schiffe mit Warp Antrieben ausgerüstet, die Reisen mit Überlichtgeschwindigkeit ermöglichen.\n\nAuf einem Schiff können nur entweder ein Warp Antrieb oder ein Sprung Antrieb installiert werden.",

    upgrade_jumpDrive_name = function(range)
        return f("Jump Antrieb %dU", range)
    end,
    upgrade_jumpDrive_description = function(range)
        return f("Der Sprungantrieb wird häufig von militärischen und paramilitärischen Gruppierungen verwendet um Überraschungsangriffe durch zu führen. Doch auch für längere Reisen ist der Sprungantrieb durchaus geeignet. Dieser Antrieb erlaubt Sprünge bis zu %dU.\n\nAuf einem Schiff können nur entweder ein Warp Antrieb oder ein Sprung Antrieb installiert werden.", range)
    end,
    ...
    upgrade_mine1_description = function(storageMalusSize)
        return f("Durch Einrichtung eines Stasisfelds kann im Lagerraum eine Mine sicher transportiert werden. Der Lagerraum wird durch dem Umbau um %d Einheiten verkleinert.", storageMalusSize)
    end,
    ...
    upgrade_powerPresets_description = function(slots)
        return f("Dieses Software Update für die Engineering Station gestattet es die Energie- und Kühlmittelverteilung in Raumschiffen zu spreichern und schnell abzurufen. Viele Engineers auf größeren Schiffen nutzen dieses System, um effizienter zu agieren und schnell auf sich verändernde Anforderungen im Energiebedarf von Raumschiffen zu reagieren.", slots)
    end,

    upgrade_shield1_name = "Shield MkI",
    upgrade_shield1_description = function(strength)
        return f("Kein Handelsschiff sollte sich in die Weiten des Alls ohne adequate Schilde wagen. Dieses Schild mit einer Nennkapazität von %d hat die vordergründige Aufgabe das Schiff for Asteroidenkolissionen zu schützen.", strength)
    end,
    ...
    upgrade_laserRefit_name = "Laser Config",
