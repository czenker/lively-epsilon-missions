...
My.Translator:register("en", {
    ...
    upgrade_shieldemp_description = function(rangeInU)
        return f("In emergency situations the ships shield can be forcefully discharged to cause EMP damage in a radius of up to %0.1fu. Its mode of operation can be compared to that of an EMP missile.", rangeInU)
    end,
    upgrade_shieldemp_label = "EMP Burst",
    upgrade_shieldemp_info_label = "Info",
    upgrade_shieldemp_info = function(rangeInU)
        return f("The EMP burst has a similar effect as an EMP Missile. The electro-magnetic pulse is generated by explosivly discharging the shields and releasing its energy damaging everything in a %0.2fu radius with emp damage. The damage dealt is dependent on distance to the target and the current level of the shields.", rangeInU)
    end,
    upgrade_shieldemp_info_current = function(minDamage, maxDamage)
        return f("Triggering the EMP burst right now would deal damage between %0.2f and %0.2f.", minDamage, maxDamage)
    end,
    upgrade_shieldemp_info_low_charge = function(minCharge)
        return f("The shields must have a charge of at least %0.2f in order to use the EMP Burst.", minCharge)
    end,
    upgrade_shieldemp_fire_label = "Fire",
    upgrade_shieldemp_error_charge = function(minCharge)
        return f("The shields must have a charge of at least %0.2f in order to use the EMP Burst!", minCharge)
    end,
    upgrade_shieldemp_error_shields_down = "Shields need to be up to fire the EMP Burst!",
})