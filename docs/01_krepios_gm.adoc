...
* **one shipyard** which is used as home for a crazy tinkerer. It is close to a nebula and has an arena close-by.
...
Player > Update DB::
Clicking that button will update the Science DB entries. Hit it if you changed, renamed or moved
any stations, nebulas, etc before starting the scenario.
